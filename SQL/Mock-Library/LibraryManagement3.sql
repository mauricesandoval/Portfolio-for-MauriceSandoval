...
CREATE PROC GetBranchDueDate @BranchName VARCHAR(20), @DueDate VARCHAR(30)
AS
SELECT B.Title, BW.Name, BW.Address, BL.DueDate
FROM BOOK_LOANS AS BL INNER JOIN BORROWER AS BW
...
WHERE BranchName LIKE @BranchName + '%' AND DueDate LIKE @DueDate + '%'
GO

EXEC GetBranchDueDate @BranchName = 'Sharpstown', @DueDate = '2016-01-04'


--5

CREATE PROCEDURE GetBranchBooksOut @BranchName nvarchar(30), @BooksOut int OUT
AS
SELECT @BooksOut = count(*)
FROM BOOK_LOANS INNER JOIN LIBRARY_BRANCH
ON BOOK_LOANS.BranchId = LIBRARY_BRANCH.BranchId
WHERE DueDate >= '2016-01-01' AND BranchName LIKE @BranchName + '%'
GO

DECLARE @BooksOut int
	EXEC GetBranchBooksOut @BranchName = 'Sharpstown', @BooksOut = @BooksOut OUTPUT
	SELECT @BooksOut

GO

DECLARE @BooksOut int
	EXEC GetBranchBooksOut @BranchName = 'Central', @BooksOut = @BooksOut OUTPUT
	SELECT @BooksOut
GO
	
DECLARE @BooksOut int
	EXEC GetBranchBooksOut @BranchName = 'City', @BooksOut = @BooksOut OUTPUT
	SELECT @BooksOut
GO
	
DECLARE @BooksOut int
	EXEC GetBranchBooksOut @BranchName = 'Eagle Heights', @BooksOut = @BooksOut OUTPUT
	SELECT @BooksOut	


--6

CREATE PROCEDURE GetBooksOutFive @BranchName nvarchar(30), @5BooksOut varchar(30) OUT
AS
SELECT @5BooksOut = count(*)
FROM BOOK_LOANS INNER JOIN BORROWER
ON BOOK_LOANS.CardNo = BORROWER.CardNo
WHERE DueDate >= '2016-01-01' AND BranchName LIKE @BranchName + '%'
GO

SELECT MAX(CardNo) FROM BOOK_LOANS;

SELECT COUNT(CardNo) FROM BOOK_LOANS;
